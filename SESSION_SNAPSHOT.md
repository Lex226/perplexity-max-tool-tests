...
### Приложение
- **Файл:** `tool-tests.html` (86KB, vanilla HTML/CSS/JS)
...
├── README.md              ← Описание проекта, таблица инструментов, инструкция
├── SESSION_SNAPSHOT.md    ← Этот файл
└── tool-tests.html        ← Загрузить вручную (86KB, см. README)
```

### Загрузка tool-tests.html
Файл нужно загрузить вручную через GitHub UI или git:
```bash
git clone https://github.com/Lex226/perplexity-max-tool-tests
# скопировать tool-tests.html в папку
git add tool-tests.html
git commit -m "feat: add full tool-tests app"
git push
```
...
- [ ] Загрузить полный `tool-tests.html` в репо
- [ ] Включить GitHub Pages для репо (Settings → Pages → main branch)
- [ ] Пройтись по чеклисту инструментов в приложении
- [ ] Добавить тесты для MCP Servers (GitHub MCP уже подключён!)
- [ ] Сравнить Comet Browser с обычным Deep Research
- [ ] Попробовать Agent API с кастомными функциями для 1С/SQL Server данных
- [ ] Расширить Space промпт под конкретные задачи BI/DWH
